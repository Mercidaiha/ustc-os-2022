...
// 定义要输出的文件
#define OUTPUT_FILE "/home/mercidaiha/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffb7cb0780;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffb7c94160;
...
static void print_seg_info(void)
{
    ...
    //                   static char global_data[100]和char hamlet_scene1[8276]的内容。
        ...
        struct vm_area_struct *vma = find_vma(mm, start_data);
        struct vm_area_struct *vma_end = find_vma(mm, end_data);
        struct vm_area_struct *end = vma_end->vm_next;
        while(vma!=end){
            unsigned long virt_addr = vma->vm_start;
            while(virt_addr<(vma->vm_end)){
                struct page * page = mfollow_page(vma,virt_addr,FOLL_GET);
                unsigned long virt_end = virt_addr + PAGE_SIZE;
                if((virt_addr >= start_data)&&(virt_end <= end_data)){
                ...
                else if((virt_addr <= start_data)&&(virt_end >= end_data)){
                    addr = kmap_atomic(page);
                    memcpy(buf, addr + start_data - virt_addr, end_data - start_data);
                    kunmap_atomic(addr);
                    curr_buf_length += (end_data - start_data);
                    flush_buf(0); 
                }
                else if((virt_addr <= start_data)&&(virt_end >= start_data)){
                    addr = kmap_atomic(page);
                    memcpy(buf, addr + start_data - virt_addr, virt_end - start_data);
                    kunmap_atomic(addr);
                    curr_buf_length += (virt_end - start_data);
                    flush_buf(0); 
                }
                else if((virt_addr <= end_data)&&(virt_end >= end_data)){
                    addr = kmap_atomic(page);
                    memcpy(buf, addr, end_data - virt_addr);
                    kunmap_atomic(addr);
                    curr_buf_length += (end_data - virt_addr);
                    flush_buf(0); 
                }
                virt_addr = virt_addr + PAGE_SIZE;
            }
        ...
        //begin and print
        struct page * page1;
        unsigned long virt_addr1 = vma->vm_start;
        while(virt_addr1 < (vma->vm_end)){
            if(start_data>=virt_addr1){
                page1 = mfollow_page(vma,virt_addr1,FOLL_GET);
                break;
            }
            virt_addr1 = virt_addr1 + PAGE_SIZE;
        }
        addr=kmap_atomic(page1);
        size_t size1 = virt_addr1 + PAGE_SIZE - start_data; 
        memcpy(buf,addr,size1);
        curr_buf_length += size1;
        flush_buf(0);

        virt_addr1 += PAGE_SIZE;
        while(virt_addr1 < (vma->vm_end)){
            struct page * page = mfollow_page(vma,virt_addr1,FOLL_GET);
                if(!IS_ERR_OR_NULL(page)){           
                    addr = kmap_atomic(page);
                    memcpy(buf, addr, PAGE_SIZE);
                    kunmap_atomic(addr);
        ...
        //end
        struct page * page2;
        unsigned long virt_addr2 = vma_end->vm_start;
        while(virt_addr2 < (vma->vm_end)){
            if(end_data < (virt_addr2 + PAGE_SIZE)){
                page2 = mfollow_page(vma_end,virt_addr2,FOLL_GET);
                break;
            }
            virt_addr2 = virt_addr2 + PAGE_SIZE;
        }



        //mid and print 
        struct vm_area_struct *vma_temp = vma->vm_next;
        while(vma_temp!=vma_end){
            unsigned long virt_addr = vma_temp->vm_start;
            while(virt_addr < (vma_temp->vm_end)){
                struct page * page = mfollow_page(vma_temp,virt_addr,FOLL_GET);
                if(!IS_ERR_OR_NULL(page)){           
                    addr = kmap_atomic(page);
                    memcpy(buf, addr, PAGE_SIZE);
                    kunmap_atomic(addr);
        ...
        //print end
        addr=kmap_atomic(page2);
        size_t size2 =end_data - virt_addr2; 
        memcpy(buf,addr,size2);
        curr_buf_length += size2;
        flush_buf(0);

        unsigned long virt_addr = vma_end->vm_start;
        while(virt_addr < virt_addr2){
            struct page * page = mfollow_page(vma_end,virt_addr,FOLL_GET);
                if(!IS_ERR_OR_NULL(page)){           
                    addr = kmap_atomic(page);
                    memcpy(buf, addr, PAGE_SIZE);
                    kunmap_atomic(addr);

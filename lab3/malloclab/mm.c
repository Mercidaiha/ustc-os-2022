...
static void *coalesce(void *bp)
{
    ...
    */
    ...
    {   
        size_t new_size = size + GET_SIZE(HDRP(NEXT_BLKP(bp)));
        char* next_bp = NEXT_BLKP(bp);
        delete_from_free_list(next_bp);
        PUT(HDRP(bp),PACK(new_size, 1 , 0));
        PUT(FTRP(bp),PACK(new_size, 1 , 0));
        add_to_free_list(bp);
    }
    else if (!prev_alloc && next_alloc) /*前块空闲，后块已分配*/
    {   
        size_t new_size = size + GET_SIZE(HDRP(PREV_BLKP(bp)));
    ...
    {   
        size_t new_size = size + GET_SIZE(HDRP(PREV_BLKP(bp))) + GET_SIZE(HDRP(NEXT_BLKP(bp)));
        char* next_bp = NEXT_BLKP(bp);
        char* prev_bp = PREV_BLKP(bp);
        delete_from_free_list(next_bp);
        delete_from_free_list(prev_bp);
...
static void* find_fit_best(size_t asize) {
    /* 
        ...
        HINT: asize 已经计算了块头部的大小
    */
    char* best = free_listp;
    char* bp = free_listp;
    size_t min = 0xffffffffffffffff;
    while(bp){
        if((GET_SIZE(HDRP(bp))>=asize) && (GET_SIZE(HDRP(bp))<min)){
            min = GET_SIZE(HDRP(bp));
            best = bp;
        }
        bp = (char*)(GET_SUCC(bp));
    }
    if((best == free_listp)&&(min == 0xffffffffffffffff))
        return NULL; // 换成实际返回值
    else return best;
}
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    if((bp_size-asize) > MIN_BLK_SIZE){
        ...
        PUT(HDRP(new_bp),PACK(bp_size - asize, 1 ,0));
        PUT(FTRP(new_bp),PACK(bp_size - asize, 1 ,0));
        ...
        PUT(HDRP(next_bp),PACK(next_bp_size, 1 ,next_alloc));
        if(!next_alloc) PUT(FTRP(next_bp),PACK(next_bp_size, 1 , next_alloc));
        user_malloc_size = user_malloc_size + bp_size - WSIZE;
    }

...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            //TODO: 打开输出文件从头写入
            tfd = open(argv[i+1],O_WRONLY|O_CREAT|O_TRUNC,0666);//0666 is essential, for authority; trunc is for beginning
            if(tfd < 0) {
...
int main() {
    ...
    while (1) {
        ...
        int cmd_count1;
        int n;
        cmd_count1 = split_string(cmdline, ";", commands1);
        /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
        /* 拆解命令行 */
        for(n=0;n < cmd_count1; n++){
            cmd_count = split_string(commands1[n], "|", commands);
            if(cmd_count == 0) {
                continue;
